...
public class SchemaStatVisitorExt extends SQLASTVisitorAdapter {
    ...
    protected final List<TableConditionExt>                    conditions     = new ArrayList<>();
    ...
    public SchemaStatVisitorExt(){
        this((DbType) null);
    }

    public SchemaStatVisitorExt(SchemaRepository repository) {
        if (repository != null) {
    ...
    public SchemaStatVisitorExt(DbType dbType){
        this(new SchemaRepository(dbType), new ArrayList<Object>());
        this.dbType = dbType;
    }

    public SchemaStatVisitorExt(List<Object> parameters){
        this((DbType) null, parameters);
    }

    public SchemaStatVisitorExt(DbType dbType, List<Object> parameters){
        this(new SchemaRepository(dbType), parameters);
        this.parameters = parameters;
    }

    public SchemaStatVisitorExt(SchemaRepository repository, List<Object> parameters){
        this.repository = repository;
        this.parameters = parameters;
        if (repository != null) {
    ...
    public List<TableConditionExt> getConditions() {
        return conditions;
    }
    ...
    protected void handleCondition(SQLExpr expr, String operator, SQLExpr... valueExprs) {
        ...
        TableConditionExt condition = new TableConditionExt(column, operator);
		condition.setExpr(expr.getParent());
		this.conditions.add(condition);
        for (TableConditionExt item : this.getConditions()) {
            if(item.getExpr()==expr){
            	return;
			}
        }
...
//        }
    ...
    public boolean visit(SQLMethodInvokeExpr x) {
		...
		TableConditionExt tc = getTableConditionExtFromFunc(x);
		if(tc!=null){
			for (Iterator<TableConditionExt> iterator = conditions.iterator(); iterator.hasNext(); ) {
				TableConditionExt existing =  iterator.next();
				if(existing.getExpr()==tc.getExpr())
					return false;
			}
			conditions.add(tc);
		}
	...
	public TableConditionExt getTableConditionExtFromFunc(SQLMethodInvokeExpr func){
		if (func.getParent() instanceof SQLMethodInvokeExpr || func.getParent() instanceof SQLSelectItem)
			return null;
		SQLVariantRefExpr variant = findArgumentColumn(func, new HashMap<>(), se -> {
			if (se instanceof SQLVariantRefExpr) {
				return (SQLVariantRefExpr) se;
			}
			return null;
		});
		if (variant == null)
			return null;
		SQLObject sibling = getLastSibling(variant);
		SQLBinaryOpExpr parent= (SQLBinaryOpExpr) sibling.getParent();
		Column col = firstColumn(sibling);
		if(col!=null){
			TableConditionExt condi = new TableConditionExt(col, parent.getOperator().toString());
			condi.setExpr(parent);
			return condi;
		}else{
			col=new Column(Constants.UNKNOWN_TABLE,func.getMethodName(),dbType);
			TableConditionExt condi = new TableConditionExt(col, parent.getOperator().toString());
			condi.setExpr(parent);
			return condi;
		}
	...
	public Column firstColumn(SQLObject so){
		SchemaStatVisitor v = SQLUtils.createSchemaStatVisitor(dbType);
		so.accept(v);
		Collection<Column> cols = v.getColumns();
		if(cols.size()>0)
	...
	private static final List<SQLBinaryOperator> conditionalBinaryOperators=Arrays.asList(new SQLBinaryOperator[]{
		...
		SQLBinaryOperator.ILike,
		SQLBinaryOperator.NotILike,
		SQLBinaryOperator.AT_AT,
		SQLBinaryOperator.SIMILAR_TO,
		SQLBinaryOperator.POSIX_Regular_Match,
		SQLBinaryOperator.POSIX_Regular_Not_Match,
		SQLBinaryOperator.Array_Contains,
		SQLBinaryOperator.Array_ContainedBy,
		SQLBinaryOperator.SAME_AS,
		SQLBinaryOperator.JSONContains,
	});

	private SQLObject getLastSibling(SQLObject parent){
		SQLObject last=parent;
		parent=parent.getParent();
		while (parent!=null){
			if(parent instanceof SQLSelectQueryBlock || parent instanceof SQLSelectItem){
				return null;
			}
			if(parent instanceof SQLBinaryOpExpr && conditionalBinaryOperators.indexOf(((SQLBinaryOpExpr) parent).getOperator())!=-1) {
				SQLBinaryOpExpr be= (SQLBinaryOpExpr) parent;
				return be.getLeft()==last?be.getRight():be.getLeft();
			}
			parent=parent.getParent();
		}
		return null;
	...
	private SQLVariantRefExpr findArgumentColumn(SQLObject se, Map<Object, Object> processed, Function<SQLObject, SQLVariantRefExpr> end) {
		if (se == null)
			return null;
		if (processed.containsKey(se))
			return null;
		processed.put(se, new Object());
		SQLVariantRefExpr ret = end.apply(se);
		if (ret != null)
			return ret;
		...
		if (p instanceof SQLBinaryOpExpr) {
			...
			SQLVariantRefExpr lr = findArgumentColumn(be.getLeft(), processed, end);
			if (lr != null)
				return lr;
			SQLVariantRefExpr rr = findArgumentColumn(be.getRight(), processed, end);
			if (rr != null)
				return rr;
		}

		if (p instanceof SQLBetweenExpr){
			SQLBetweenExpr bw= (SQLBetweenExpr) p;
			SQLVariantRefExpr lr = findArgumentColumn(bw.getBeginExpr(), processed, end);
			if (lr != null)
				return lr;
			SQLVariantRefExpr rr = findArgumentColumn(bw.getEndExpr(), processed, end);
			if (rr != null)
				return rr;
		}
...
//            stat.incrementSh
    ...
    public static SchemaStatVisitorExt createSchemaStatVisitorExt(DbType dbType){
		SchemaRepository repository = new SchemaRepository(dbType);
		SchemaStatVisitorExt v = new SchemaStatVisitorExt(repository);
		return v;
	}
}
