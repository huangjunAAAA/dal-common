...
public class JdbcTCollectionImpl implements CollectionDao {
    ...
    public List<Object[]> getDataListMulti(String listName, Object[] params, Integer start, Integer count, boolean forceMaster) throws Exception {
        ...
        List<Object[]> ret;
        if(de.isNamedParam()){
            SqlParameterSource nds = createSqlParamSource(params);
            ret = queryDB(() -> namedParameterJdbcTemplate.query(de.getSql(),nds,createRowMapper(de)), forceMaster);
        }else{
            ret = queryDB(() -> jdbcTemplate.query(de.getSql(),params,createRowMapper(de)), forceMaster);
        }
        if (logger.isDebugEnabled())
    ...
    private RowMapper<Object[]> createRowMapper(final DataEntry de){
        RowMapper<Object[]> rm=new RowMapper<Object[]>() {
            @Override
            public Object[] mapRow(ResultSet resultSet, int i) throws SQLException {
                Object[] vals=new Object[de.getValueProperties().size()];
                int idx=0;
                for (Iterator<Map.Entry<String, Class>> iterator = de.getValueProperties().entrySet().iterator(); iterator.hasNext(); ) {
                    Map.Entry<String, Class> outParams =  iterator.next();
                    vals[idx++]=resultSet.getObject(outParams.getKey(),outParams.getValue());
                }
                return vals;
            }
        };
    ...
    private SqlParameterSource createSqlParamSource(Object[] params){
        MapSqlParameterSource nps=new MapSqlParameterSource();
        for (int i = 0; i < params.length; i++) {
            Object param = params[i];
            String np = "p" + i++;
            nps.addValue(np,param);
        }
        return nps;
    }
    ...
    public <T> List<T> getDataListSingle(String listName, Object[] params, Integer start, Integer count, Integer colIdx, boolean forceMaster) throws Exception {
        ...
        setIndexedQueryScalar(query, de, sessionFactory.getTypeHelper(), colIdx);
    ...
    public Integer countDataList(String listName, Object[] params, boolean forceMaster) throws Exception {
        ...
        query.addScalar("cc", sessionFactory.getTypeHelper().basic(Integer.class));
    ...
    public Object[] getDataMapMulti(String mapName, Object[] params, boolean forceMaster) throws Exception {
        ...
        setParameter(query, params, null, null);
        setQueryScalar(query, de, sessionFactory.getTypeHelper());
