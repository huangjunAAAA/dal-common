...
public class SchemaStatVisitorExt extends SQLASTVisitorAdapter {
    ...
    protected void handleCondition(SQLExpr expr, String operator, SQLExpr... valueExprs) {
        ...
        TableConditionExt condition = null;
        for (TableConditionExt item : this.getConditions()) {
            if (item.getColumn().equals(column) && item.getOperator().equals(operator)) {
                condition = item;
				condition.addExpr(expr.getParent());
                break;
            }
        }

        if (condition == null) {
            condition = new TableConditionExt(column, operator);
            condition.addExpr(expr.getParent());
...
//        }
	...
	public TableConditionExt getTableConditionExtFromFunc(SQLMethodInvokeExpr func){
		...
		TableStat.Column col = firstColumn(sibling);
		if(col!=null){
			TableConditionExt condi = new TableConditionExt(col, parent.getOperator().toString());
			condi.addExpr(parent);
			return condi;
		}else{
			col=new TableStat.Column(Constants.UNKNOWN_TABLE,func.getMethodName(),dbType);
			TableConditionExt condi = new TableConditionExt(col, parent.getOperator().toString());
			condi.addExpr(parent);
			return condi;
		}
	...
	public TableStat.Column firstColumn(SQLObject so){
	...
	private SQLObject getLastSibling(SQLObject parent){
		...
		parent=parent.getParent();
		while (!(parent instanceof SQLBinaryOpExpr)&&!(parent instanceof SQLSelectQueryBlock) &&!(parent instanceof SQLSelectItem)){
			parent=parent.getParent();
		}
